...
public class EbirdComparisonTool implements Serializable {
  ...
  public static void main(String[] args) {
    // TODO: args
    EbirdComparisonTool.builder().hiveDB(args[0]).targetFile(args[1]).build().run();
  }
  ...
  public void run() {
    try (SparkSession spark =
      ...
      Column rawJoinColumn = when(baseColumn.startsWith(ID_PREFIX), substring(baseColumn, ID_PREFIX.length() + 1, Integer.MAX_VALUE))
        .otherwise(baseColumn);


      Dataset<Row> joined =
        rawTable.alias(RAW_TABLE_ALIAS)
          .join(
      ...
      selectedColumns.add(
        coalesce(col(RAW_TABLE_ALIAS + "." + rawJoinColumn), col(PROD_TABLE_ALIAS + "." + prodJoinColumn))
          .alias("join_key"));

      for (String columnName : rawTable.columns()) {
        selectedColumns.add(col(RAW_TABLE_ALIAS + "." + columnName).alias(RAW_TABLE_ALIAS + columnName));
      }

      for (String columnName : prodTable.columns()) {
        selectedColumns.add(col(PROD_TABLE_ALIAS + "." + columnName).alias(PROD_TABLE_ALIAS + columnName));
      }
